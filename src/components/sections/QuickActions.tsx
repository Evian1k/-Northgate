...
import {
  FileText, Download, Wallet, UserCircle, MonitorPlay, Search,
  ArrowUpRight, X, type LucideIcon,
} from "lucide-react";
...
const actions: Action[] = [
  { title: "Apply Online", desc: "Start your journey today", icon: FileText, href: "/apply", accent: true },
  { title: "Download Brochure", desc: "2026 prospectus (PDF)", icon: Download, href: "/api/brochure", external: true },
  { title: "Fee Structure", desc: "Tuition & funding options", icon: Wallet, href: "/fees" },
  { title: "Student Portal", desc: "Grades, results & e-learning", icon: UserCircle, href: "/portal" },
  { title: "Virtual Tour", desc: "Explore campus in 3D", icon: MonitorPlay, modal: "tour" },
        ...
        <div className="mx-auto max-w-7xl">
          ...
          >
            {actions.map((a) => {
              ...
              const className = `group relative block h-full rounded-2xl p-4 sm:p-5 transition-all hover:-translate-y-1 ${
                a.accent
              ...
              if (a.modal === "tour") {
                return (
                  <motion.div key={a.title} variants={staggerItem}>
                    <button onClick={() => setTourOpen(true)} className={className + " w-full text-left"}>
                      {content}
                    </button>
                  </motion.div>
                );
