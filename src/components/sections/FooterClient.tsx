...
export function FooterClient({ siteName, phone, email, address }: { siteName: string; phone: string; email: string; address: string }) {
  ...
  const subscribe = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        toast({ title: "Subscribed!", description: "You'll receive Northgate updates soon." });
        setEmail("");
      } else {
        toast({ title: "Could not subscribe", description: data.error || "Try again later.", variant: "destructive" });
      }
    } catch {
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
        ...
        <div className="mt-12 pt-8 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-2">
            {socials.map((s) => (
              <a key={s.label} href={s.href} aria-label={s.label} className="grid place-items-center h-10 w-10 rounded-full bg-white/5 hover:bg-gold hover:text-navy text-white/70 transition-colors">
