...
export default async function StudentDashboardPage() {
  ...
  const now = new Date();
  const [
    enrollments, pendingAssessments, poeRequests, poeSubmissions,
    attendanceRecords, fees, examCard, activeLoans, unreadNotifs,
    upcomingDeadlines, releasedResults, hostelAlloc,
  ] = await Promise.all([
    ...
    db.assessment.count({
      where: { type: "POE_REQUEST", unit: { enrollments: { some: { studentId: student.id } } }, dueDate: { gte: now } },
    }),
