...
export default async function AssessmentsPage() {
  ...
  if (isDemoUser(user.id)) return <AssessmentsClient assessments={demoAssessments} />;
  try {
    const { db } = await import("@/lib/db");
    const { getCurrentStudent } = await import("@/lib/student-session");
    const student = await getCurrentStudent();
    if (!student) return <div>Unauthorized</div>;
    const assessments = await db.assessment.findMany({ where: { unit: { enrollments: { some: { studentId: student.id } } } }, orderBy: { dueDate: "asc" }, include: { unit: { select: { code: true, title: true } }, submissions: { where: { studentId: student.id }, select: { id: true, status: true, marks: true, feedback: true, submittedAt: true } } }, take: 100 });
    ...
    return <AssessmentsClient assessments={demoAssessments} />;
  }
}
