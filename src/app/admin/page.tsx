...
import { getDemoAdminStats, demoUsers } from "@/lib/demo-data";
...
export default async function AdminDashboardPage() {
  ...
  // Real admin — try DB with fallback to demo
  try {
    const { db } = await import("@/lib/db");
    const [counts, recentActivity, recentApplications, recentMessages] = await Promise.all([
      {
      ...
      db.auditLog.findMany({ orderBy: { createdAt: "desc" }, take: 10, include: { user: { select: { name: true, email: true } } } }),
      db.application.findMany({ orderBy: { createdAt: "desc" }, take: 5, include: { programme: { select: { title: true, code: true } } } }),
    ...
    return (
      <DashboardClient
        counts={counts}
        recentActivity={recentActivity.map((a) => ({ id: a.id, action: a.action, resource: a.resource, createdAt: a.createdAt.toISOString(), user: a.user?.name || "System" }))}
        recentApplications={recentApplications.map((a) => ({ id: a.id, reference: a.reference, firstName: a.firstName, lastName: a.lastName, email: a.email, status: a.status, createdAt: a.createdAt.toISOString(), programme: a.programme?.title || "—" }))}
        recentMessages={recentMessages.map((m) => ({ id: m.id, name: m.name, email: m.email, subject: m.subject, status: m.status, createdAt: m.createdAt.toISOString() }))}
        trend={trend}
      />
    );
  } catch {
    // DB failed — use demo data
    const stats = getDemoAdminStats();
    return (
      <DashboardClient
        counts={stats.counts}
